...
public class UserAPIRoutes extends Router {
    ...
    @GetMapping(value = "/findNotExistAvatars")
    @ResponseBody
    public void findNotExistAvatars() {
        avatarRepository.find(null, null).forEach(document -> {
            userRepository.updateMany(and(
                    exists("avatar_id", true),
                    eq("avatar_id", document.getObjectId("_id"))
            ), set("pic", document.getString("file")));
        });
    }
